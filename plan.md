...
- AI 结论是否有证据、可追溯、可拒绝回答，且系统始终只提供建议。
...
- 工具不登录支付宝，不设计交易适配器、申赎执行接口或账户控制能力。
...
- 展示任务状态、数据源状态和历史报告；
- 设置调用次数和 Token/费用上限；
- 影子运行：先观察和评估建议，所有实际操作始终由用户独立完成。

### 3.2 第一版明确不做

- 登录支付宝或保存支付宝凭据；
- 下单、申购、赎回或任何形式的仓位执行功能；
- 分钟级或实时行情；
- 全网全文抓取；
- 任意域名 PDF、压缩包和附件解析；
- 大规模社交媒体抓取；
...
- 硬性风险边界和建议强度由确定性规则计算；
- AI 建议超出风险边界时，以规则引擎的保守建议为准；
- AI 无法确认时必须返回 `unknown` 或 `INSUFFICIENT_DATA`；
- 系统中不存在可授予 AI 的交易权限或交易接口。
...
- 定义建议标签枚举：`INSUFFICIENT_DATA`、`OBSERVE`、`HOLD`、`SMALL_ADD`、`PAUSE_ADDING`、`REBALANCE`、`REDUCE`；这些标签只用于报告展示，不连接任何交易执行能力；
...
- 数据库文件权限最小化；
- 云端数据库不开放公网；
- 备份功能保持可选；只有启用备份时才要求加密，不作为个人 Demo 阻塞项；
- 日志中不记录完整持仓和原始密钥；
- 删除工作区时能删除对应敏感数据。
...
- 建立域名白名单和来源级策略；
- 建立来源级限速、熔断和健康状态；
- 本机阶段不专门创建低权限操作系统账号，部署阶段再按实际运行环境决定；
- 将原始内容写入隔离区，不直接进入报告。

### 第一版安全取舍

- 个人本机 Demo 只实现防止错误联网、内网访问、任务卡死和内存失控的最低必要保护；
- 加密备份保持可选，独立操作系统账号、容器沙箱、mTLS、云密钥管理和多用户权限后置；
- GDELT 只保存元数据和原始链接；
- 仅对明确白名单的官方域名抓正文；
...
- 实现单主题仓位限制；
- 实现目标仓位缺口；
- 实现单次加仓建议上限和减仓建议的保守边界；
- 实现基金费用和持有期约束；
- 输出建议标签、建议强度、非强制参考边界、原因和规则版本；
...
- AI 不能修改仓位上限；
- AI 不能扩大本地风险边界；
- 决策引擎只产生可解释建议，不包含交易适配器、账户凭据或申赎执行代码；
- 极端数据和空值安全处理；
- 所有决策记录输入快照。
...
## 步骤 19：可选部署、网络隔离和跨节点同步

> 执行门槛：本步骤不再是本机个人 Demo 的必做阶段。只有用户在完整闭环验证后明确决定
> 购买云服务器、开放公网或建立境内外节点时才启动；否则继续使用 Windows 本机运行。
...
> 范围限制：只验证个人 Demo 已实现的联网、外部内容、AI 输出、费用上限和失败降级边界；
> 不开展企业渗透测试、合规认证、多租户攻防或云基础设施专项审计。
...
- 影子运行和后续使用期间都只生成建议，实际操作始终由用户独立决定和完成。
...
| AI 输出越过风险限制 | 产生过激建议 | DecisionPolicy 降级为保守建议 | 人工复核工作流 |
...
依赖关系上，步骤 0～7 是不可跳过的基础；步骤 8 和步骤 9 可以并行开发；步骤 13 可以先使用 MockAIProvider；步骤 18 可以在后端接口稳定后再完善视觉设计；步骤 19 只有用户明确需要云端持续运行时才进行。
...
16. 如用户启用备份，备份恢复流程经过验证；未启用时不阻塞个人 Demo；
17. AI 和抓取费用存在每日上限；
18. 工具不包含交易适配器、申赎执行接口或账户控制能力；
19. 固定功能和安全测试集全部通过；
