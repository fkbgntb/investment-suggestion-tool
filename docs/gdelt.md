...
不会伪造空白新闻。GDELT 偶尔会响应较慢或限流，遇到 `TIMEOUT`/`RATE_LIMITED` 时应等待下一个周期，
而不是高频重试。

GDELT DOC 2.0 是公共研究接口，响应时间可能明显高于普通网页，且会拒绝间隔不足 5 秒的连续请求。
该来源单独使用 15 秒连接、60 秒读取、90 秒总超时和 6 秒最小请求间隔；域名白名单、内容类型、
2 MB 响应上限和未批准地址拦截保持不变。
单次超时只会把 GDELT 标记为降级，不影响 SEC 等其他来源。

为避免向公共接口提交昂贵的超长布尔查询，查询生成器按 Topic 轮询选择代表词，最多保留 16 个
去重后的高信号词。这样不会让第一个 Topic 独占关键词额度，也不会把 10 个子主题扩张成 40 个
OR 条件。
