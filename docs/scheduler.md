...
只有窗口产生新文档时，才向数据库 `scheduled_tasks` 表写入 `process-new-documents`。同一调度进程
随后完成清洗、相关性筛选、证据抽取、评分和报告任务消费；任一步失败都会保留可重试状态，不会把
半成品报告发布。没有新评分证据时记录明确的跳过原因，不制造重复报告。

`daily-summary` 每个 UTC 日期最多排队一次，并由同一任务消费者生成观察报告或明确跳过；过期正文
清理也每天最多执行一次。任务和报告分别使用稳定摘要去重，重启或重复触发不会产生相同报告。
