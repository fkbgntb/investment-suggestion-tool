"""One bounded GDELT collection run with durable summary, cursor, and health."""
...
from app.collectors.gdelt import GDELTAdapter, GDELTResponseError, discovered_to_raw_document
...
class GDELTCollectionService:
    ...
    async def run(
        ...
        *,
        since: datetime,
        until: datetime,
    ) -> GDELTCollectionOutcome:
        source = self.sources.get(source_id)
        if source.adapter_name != "gdelt-doc":
            raise SourceConflict("source is not configured for the GDELT adapter")
        ...
        topics = {topic.topic_id: topic for topic in configuration.topics if topic.enabled}
        if not topics:
        ...
        crawl_run_id = str(uuid5(NAMESPACE_URL, f"gdelt:{self.workspace_id}:{request_digest}"))
        ...
        used_today = self.documents.count_since(source_id=source_id, since=start_of_day)
        remaining = self.max_documents_per_day - used_today
        if remaining <= 0:
        ...
        adapter = GDELTAdapter(
            self.http_client,
            topics,
            max_records=min(self.max_records, remaining),
            max_documents_per_day=remaining,
        )
        try:
        ...
        outcome = GDELTCollectionOutcome(
            crawl_run_id=crawl_run_id,
            status="SUCCEEDED",
            discovered_count=len(result.documents),
            created_count=created_count,
            duplicate_count=len(result.documents) - created_count,
            truncated=adapter.last_truncated or len(result.documents) >= remaining,
